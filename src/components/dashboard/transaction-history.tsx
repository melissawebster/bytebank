...
export default function TransactionHistory({ className }: Props) {
  return (
    <div
      className={twMerge(
        "flex flex-col items-left md:items-center w-full rounded-md bg-white p-6",
        className
      )}
    >
      <div className="w-full flex flex-col items-center gap-y-6 max-w-[250px]">
        ...
        {list.map((item, index) => (
          <div
            className="flex justify-between w-full border-b border-green-400 py-2"
            key={index}
          >
            <div className="">
            ...
            <div className="flex h-full text-gray-500">{item.data}</div>
          </div>
        ))}
      </div>
    </div>
